...
Das Setup erstellt `.venv` im Projektverzeichnis (überschreibbar über `LHA_VENV`) mit
`python3 -m venv --system-site-packages`, das heisst zusätzlich zu den venv-eigenen Paketen sind
auch systemweit installierte Python-Pakete sichtbar. Darin werden `pip`, `setuptools` und `wheel`
aktualisiert und der Agent editierbar installiert (`pip install --no-build-isolation -e .`). Das
Setup fragt bei vorhandenem Ollama interaktiv nach `qwen3:8b`, `qwen3:14b`, beiden oder keinem
Modell (nicht-interaktiv steuerbar über `LHA_INSTALL_MODELS=qwen3:8b`, `...,qwen3:14b` oder
`none`), und prüft bei installiertem Vagrant die Eigentümerschaft privater Schlüssel. Verweigert
die Ausführung als root; `.venv` muss dem aufrufenden Benutzer gehören und beschreibbar sein.
Danach:
...
`start-gui.sh` und `restart-gui.sh` starten die GUI direkt über `.venv/bin/hardening-agent` und
brechen mit einem Hinweis auf `install-local.sh` ab, falls dieser Pfad fehlt. Zusätzlich prüft
`start-gui.sh` vor dem Start die Vagrant-Schlüsselzugriffsart: bei root-eigenen Schlüsseln startet
es die GUI automatisch über `su` mit `--allow-root --no-browser`, sonst direkt als normaler
Benutzer.
